...
RANK() OVER(PARTITION BY department ORDER BY salary DESC) Rank_N
FROM employees)
WHERE Rank_N=5;
...
NTILE(10) OVER(ORDER BY salary DESC) AS GROUP_2
...
SUM(Salary) OVER(PARTITION BY department 
				 ORDER BY hire_date) AS dept_TOTAL_SALARY
FROM employees
...
region_id
COUNT(*) OVER(PARTITION BY region_id) dept_count,
FROM employees
...
SELECT first_name, hire_date, Salary, 
SUM(Salary) OVER(ORDER BY hire_date
				 ROW BETWEEN 2 PRECEDING AND CURRENT ROW) AS running_total
FROM employees

-- 7.3: Find the running average
SELECT first_name, hire_date, Salary, 
AVG(Salary) OVER(ORDER BY hire_date
				 ROW BETWEEN 2 PRECEDING AND CURRENT ROW) AS running_AVG
FROM employees
...
FIRST_VALUE(department) OVER(ORDER BY department ASC
							 RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS LAST_department,
FROM departments;
